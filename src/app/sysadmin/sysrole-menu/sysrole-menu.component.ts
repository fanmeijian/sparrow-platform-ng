...
@Component({
  selector: 'app-sysrole-menu',
  templateUrl: './sysrole-menu.component.html',
  styleUrls: ['./sysrole-menu.component.css']
})
export class SysroleMenuComponent implements OnInit {
  ...
  constructor(private menuService: MenuService, private sysroleService: SysroleService, public dialogRef: MatDialogRef<UserMenuComponent>,
  ...
  getCheckValues() {
    ...
    userMenus.sysroleId=this.dialogData._links.self.href.split('/').splice(-1)[0];
    console.log(this.dialogData._links.self.href.split('/').splice(-1))
    userMenus.menus=this.checklistSelection.selected;
    this.sysroleService.saveSysroleMenus(userMenus).subscribe();
  }
